...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const tagline = localize(settings.tagline, settings.tagline_hu, "hu");
  const about = localize(settings.about_text, settings.about_text_hu, "hu");
  return {
  ...
  return (
    <html lang="hu" className={`${bebasNeue.variable} ${vollkorn.variable} h-full antialiased`}>
      <body className="flex min-h-full flex-col bg-parchment">
        <SiteHeader locale="hu" settings={settings} />
